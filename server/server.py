...
class Server(object):
    ...
    def _summary_(self):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write("Max HPU Memory Usage / round   --->   {:.2f} MB \n".format(np.max(self.max_vram)))
    ...
    def train(self):
        ...
        for t in range(self.args.comm_rounds):
            ...
            max_vram = torch.hpu.max_memory_allocated(self.device) / (1024 * 1024 * 1024)
            print("            ----    Time: {:.2f}s, Max HPU Memory: {:.2f} MB".format(self.time[t], max_vram), flush=True)
            self.max_vram[t] = max_vram
